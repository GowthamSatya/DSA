...
class Solution
{
    Node *buildTreeHelper(int pre[], int &idx, int n, int mini, int maxi)
    {
        if (idx >= n)
            return NULL;
        ...
        nd->left = buildTreeHelper(pre, idx, n, mini, x);
        nd->right = buildTreeHelper(pre, idx, n, x, maxi);
...
public:
    ...
    Node *post_order(int pre[], int size)
    {
        // code here
        int i = 0;
        return buildTreeHelper(pre, i, size, INT_MIN, INT_MAX);
    }
};
